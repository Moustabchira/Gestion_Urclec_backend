...
async getAllMouvements(filter?: any) {
  const mouvements = await prismaClient.mouvementEquipement.findMany({
    where: filter,
  ...
  return await Promise.all(
    mouvements.map(async (m) => {
      if (m.type === "RETOUR_REPARATION") {
        const reparationOriginal = await prismaClient.mouvementEquipement.findFirst({
          where: { equipementId: m.equipementId, type: "REPARATION" },
          orderBy: { createdAt: "desc" },
          include: { initiateur: true },
        });
        return { ...m, reparationOriginal };
      }
      return m;
    })
  );

...
async envoyerEnReparation(data: {
  ...
  if (!["EN_PANNE"].includes(equipement.etat)) {
...
async confirmerReception(mouvementId: number, confirmeParId: number) {
  ...
  // ✅ Réception d’un transfert ou affectation
  if (["TRANSFERT", "AFFECTATION"].includes(mouvement.type)) {
    if (mouvement.responsableDestinationId !== confirmeParId) {
      throw new Error("Seule la personne destinataire de l’équipement peut confirmer la réception");
    }
